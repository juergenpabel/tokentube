...
int pba_user_loadkey(tt_library_t* library, const char* user, size_t user_size, const char* pass, size_t pass_size, const char* key_name, char* key, size_t* key_size) {
	...
	if( library->api.user.execute_loadkey( user, pass, key_name, key, key_size ) != TT_OK ) {
