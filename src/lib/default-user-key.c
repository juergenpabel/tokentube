...
int  default__impl__user_key_crypt_impl(tt_cryptmode_t mode, const char* username, const char* password, tt_user_t* user, size_t key_offset) {
	...
	cipher_id = gcry_cipher_map_name( user->crypto.cipher );
	if( gcry_cipher_algo_info( cipher_id, GCRYCTL_GET_KEYLEN, NULL, &key_size ) != GPG_ERR_NO_ERROR ) {
		TT_LOG_ERROR( "library/crypto", "gcry_cipher_algo_info() failed for '%s' in %s()", user->crypto.cipher, __FUNCTION__ );
		return TT_ERR;
	}
	if( key_size > TT_KEY_BITS_MAX/8 ) {
		TT_LOG_ERROR( "library/crypto", "gcry_cipher_algo_info() return key size too big for '%s' in %s()", user->crypto.cipher, __FUNCTION__ );
		return TT_ERR;
	}
