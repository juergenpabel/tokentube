...
static int exec__exec( cfg_t* cfg, const char* event, const char* identifier ) {
	...
	if( section == NULL ) {
		g_self.library.api.runtime.log( TT_LOG__ERROR, "plugin/exec", "cfg_getsec() failed for event '%s'", event );
		return TT_ERR;
	}
	section = cfg_gettsec( section, "event", event );
	if( section == NULL ) {
		g_self.library.api.runtime.log( TT_LOG__INFO, "plugin/exec", "no configuration for event '%s'", event );
		return TT_OK;
	}
	...
	switch( pid ) {
		...
		default:
			...
			if( cfg_getbool( section, "detach" ) == cfg_false ) {
				...
				waitpid( pid, NULL, 0 );
				g_self.library.api.runtime.debug( TT_DEBUG__VERBOSITY3, "plugin/exec", "...child (pid=%d) finished in exec__exec()", pid );
			} else {
				g_self.library.api.runtime.debug( TT_DEBUG__VERBOSITY3, "plugin/exec", "letting child (pid=%d) do on its own in exec__exec()", pid );
			}
			result = TT_OK;
	}
	return result;
...
static void exec__event__user_created(const char* identifier) {
	g_self.library.api.runtime.log( TT_LOG__INFO, "plugin/exec", "event 'USER_CREATED' received for identifier '%s'", identifier );
	exec__exec( g_cfg, "USER_CREATED", identifier );
}


static void exec__event__user_updated(const char* identifier) {
	g_self.library.api.runtime.log( TT_LOG__INFO, "plugin/exec", "event 'USER_UPDATED' received for identifier '%s'", identifier );
	exec__exec( g_cfg, "USER_UPDATED", identifier );
}


static void exec__event__user_verified(const char* identifier) {
	g_self.library.api.runtime.log( TT_LOG__INFO, "plugin/exec", "event 'USER_VERIFIED' received for identifier '%s'", identifier );
	exec__exec( g_cfg, "USER_VERIFIED", identifier );
}


static void exec__event__user_deleted(const char* identifier) {
	g_self.library.api.runtime.log( TT_LOG__INFO, "plugin/exec", "event 'USER_DELETED' received for identifier '%s'", identifier );
	exec__exec( g_cfg, "USER_DELETED", identifier );
}


static void exec__event__otp_created(const char* identifier) {
	g_self.library.api.runtime.log( TT_LOG__INFO, "plugin/exec", "event 'OTP_CREATED' received for identifier '%s'", identifier );
	exec__exec( g_cfg, "OTP_CREATED", identifier );
}


static void exec__event__otp_applied(const char* identifier) {
	g_self.library.api.runtime.log( TT_LOG__INFO, "plugin/exec", "event 'OTP_APPLIED' received for identifier '%s'", identifier );
	exec__exec( g_cfg, "OTP_APPLIED", identifier );
}


static void exec__event__otp_deleted(const char* identifier) {
	g_self.library.api.runtime.log( TT_LOG__INFO, "plugin/exec", "event 'OTP_DELETED' received for identifier '%s'", identifier );
...
static int configure(const char* filename) {
	if( cfg_parse( g_cfg, filename ) != 0 ) {
		g_self.library.api.runtime.log( TT_LOG__ERROR, "plugin/exec", "cfg_parse() failed for '%s'", filename );
		return TT_ERR;
	}
